...
CREATE TABLE IF NOT EXISTS house_senate_current_campaigns (
    cand_id text, -- CAND_ID,Candidate identification,1,N,VARCHAR2 (9),H8VA01233
    cand_name text, -- CAND_NAME,Candidate name,2,Y,VARCHAR2(200),Martha Washington
    cand_ici text, -- CAND_ICI,Incumbent challenger status,3,Y,VARCHAR2(1),I
    pty_cd text, -- PTY_CD,Party code,4,Y,VARCHAR2(1),NON
    cand_pty_affiliation text, -- CAND_PTY_AFFILIATION,Party affiliation,5,Y,VARCHAR2(3),NON
    ...
    cand_office_st text, -- CAND_OFFICE_ST,Candidate state,19,Y,VARCHAR2(2),VA
    cand_office_district text, -- CAND_OFFICE_DISTRICT,Candidate district,20,Y,VARCHAR2(2),01
    spec_election text, -- SPEC_ELECTION,Special election status,21,Y,VARCHAR2(1),Election result data included in 1996-2006 files only.,W
    prim_election text, -- PRIM_ELECTION,Primary election status,22,Y,VARCHAR2(1),Election result data included in 1996-2006 files only.,L
    run_election text, -- RUN_ELECTION,Runoff election status,23,Y,VARCHAR2(1),Election result data included in 1996-2006 files only.,W
    gen_election text, -- GEN_ELECTION,General election status,24,Y,VARCHAR2(1),Election result data included in 1996-2006 files only.,L
    ...
    cvg_end_dt text, -- CVG_END_DT,Coverage end date,28,Y,DATE(MM/DD/YYYY),Through date,10/19/2018
